 ...
 */
//include(dirname(__FILE__).'/secure/password_protect.php');

// Hoang : redirect to secure url when access to unsecure url
$protocol = (!empty($_SERVER['HTTPS']) && $_SERVER['HTTPS'] !== 'off' || $_SERVER['SERVER_PORT'] == 443) ? "https://" : "http://";
$domainName = $_SERVER['HTTP_HOST'].$_SERVER["REQUEST_URI"];

$domain = explode('/', $_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']);
if(count($domain) == 3 || count($domain) == 4){
    if($domain[1] === "checkout" && $domain[2] === "onepage"){
        if($protocol == 'http://'){
