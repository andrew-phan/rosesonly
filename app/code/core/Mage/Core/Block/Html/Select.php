...
class Mage_Core_Block_Html_Select extends Mage_Core_Block_Abstract
{
    ...
    protected function _toHtml()
    {
        ...
        foreach ($this->getOptions() as $key => $option) {
            ...
            $address = Mage::getModel('customer/address')->load($value);
            $plus='';
            if($value && $address!=null){
                $plus = '<input type="hidden" id="firstname-'.$value.'" value="'.$address->getData('firstname').'"/>';
                $plus .= '<input type="hidden" id="lastname-'.$value.'" value="'.$address->getData('lastname').'"/>';
                $plus .= '<input type="hidden" id="company-'.$value.'" value="'.$address->getData('company').'"/>';
                $plus .= '<input type="hidden" id="street1-'.$value.'" value="'.$address->getStreet1().'"/>';
                $plus .= '<input type="hidden" id="street2-'.$value.'" value="'.$address->getData('street2').'"/>';
                $plus .= '<input type="hidden" id="city-'.$value.'" value="'.$address->getData('city').'"/>'; 
                $plus .= '<input type="hidden" id="postcode-'.$value.'" value="'.$address->getData('postcode').'"/>'; 
                $plus .= '<input type="hidden" id="telephone-'.$value.'" value="'.$address->getData('telephone').'"/>';                
            }
            $html.= $plus;
        }
        }catch(Exception $ex){
