...
class Mage_Catalog_Model_Resource_Product_Option_Value_Collection extends Mage_Core_Model_Resource_Db_Collection_Abstract {
    ...
    public function addTitlesToResult($storeId) {
        ...
        $priceExpr = $adapter->getCheckSql(
                'store_value_price.price IS NULL', 'default_value_price.price', 'store_value_price.price'
        );
        $priceTypeExpr = $adapter->getCheckSql(
                'store_value_price.price_type IS NULL', 'default_value_price.price_type', 'store_value_price.price_type'
        );
        $titleExpr = $adapter->getCheckSql(
                'store_value_title.title IS NULL', 'default_value_title.title', 'store_value_title.title'
        );
        ...
        $this->getSelect()
                ->joinLeft(
                        array('default_value_price' => $optionTypePriceTable), $joinExprDefaultPrice, array('default_price' => 'price', 'default_price_type' => 'price_type')
                )
                ->joinLeft(
                        array('store_value_price' => $optionTypePriceTable), $joinExprStorePrice, array(
                    'store_price' => 'price',
                ...
                ->join(
                        array('default_value_title' => $optionTitleTable), 'default_value_title.option_type_id = main_table.option_type_id', array('default_title' => 'title')
                )
                ->joinLeft(
                        array('store_value_title' => $optionTitleTable), $joinExprTitle, array(
                    'store_title' => 'title',
                    'title' => $titleExpr)
                )
    ...
    public function addTitleToResult($storeId) {
        ...
        $this->getSelect()
                ...
                ->joinLeft(
                        array('store_value_title' => $optionTitleTable), $joinExpr, array(
                    'store_title' => 'title',
                    'title' => $titleExpr
                        )
                )
    ...
    public function addPriceToResult($storeId) {
        ...
        $priceExpr = null;
        if (Mage_Core_Model_App::ADMIN_STORE_ID == $storeId)
            ...
            $priceExpr = $this->getConnection()
                    ->getCheckSql('(main_table.special_from IS NULL OR main_table.special_from <= CURDATE())
                        AND (main_table.special_to IS NULL OR main_table.special_to >= CURDATE())
                        AND (main_table.special_from IS NOT NULL OR main_table.special_to IS NOT NULL)',
                    //'default_value_price.price', 
                    'main_table.special_price', 'default_value_price.price');
        ...
        $this->getSelect()
                ->joinLeft(
                        array('default_value_price' => $optionTypeTable), $joinExprDefault, array(
                    'default_price' => 'price',
                    'default_price_type' => 'price_type'
                        )
                )
                ->joinLeft(
                        array('store_value_price' => $optionTypeTable), $joinExprStore, array(
                    'store_price' => 'price',
                    'store_price_type' => 'price_type',
                    'price' => $priceExpr,
                    'price_type' => $priceTypeExpr
