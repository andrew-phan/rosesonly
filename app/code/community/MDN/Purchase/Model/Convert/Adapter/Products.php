    ...
    public function load() {
        ...
        $this->_collection = Mage::getModel('Catalog/Product')
                        ...
                        ->addExpressionAttributeToSelect('real_notify_stock_qty',
                                'if(`at_stock_qty`.`use_config_notify_stock_qty` = 0, `at_stock_qty`.`notify_stock_qty`, ' . $DefaultNotifyStockQty . ')',
                                array())
                        ->addExpressionAttributeToSelect('qty_needed',
                                '-(`at_stock_qty`.`qty` - {{ordered_qty}} - if(`at_stock_qty`.`use_config_notify_stock_qty` = 0, `at_stock_qty`.`notify_stock_qty`, ' . $DefaultNotifyStockQty . '))',
                                array('ordered_qty'))
