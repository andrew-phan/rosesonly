    ...
    {           
    	...
    	$select = $this->getSelect()
            ...
            ->joinLeft(array('catalog_product_name'=>$this->getTable('CatalogProductVarchar')),
                          'catalog_product_name.entity_id=catalog_product.entity_id and catalog_product_name.store_id = 0 and catalog_product_name.attribute_id = '.mage::getModel('Purchase/Constant')->GetProductNameAttributeId(),
                          array('name' => 'value'))            
            ->joinLeft(array('catalog_product_manufacturer'=>$this->getTable('CatalogProductInt')),
                          'catalog_product_manufacturer.entity_id=purchase_order_product.pop_product_id and catalog_product_manufacturer.store_id = 0 and catalog_product_manufacturer.attribute_id = '.mage::getModel('Purchase/Constant')->GetProductManufacturerAttributeId(),
                          array('manufacturer' => 'value'))            
            ->where('purchase_order.po_status=? and (pop_qty - pop_supplied_qty) > 0', MDN_Purchase_Model_Order::STATUS_WAITING_FOR_DELIVERY)
