...
class MDN_Purchase_ProductsController extends Mage_Adminhtml_Controller_Action {
    ...
    public function LinkSupplierAction() {
        ...
        $collection = Mage::getModel('Purchase/ProductSupplier')
                ->getCollection()
                ->addFieldToFilter('pps_product_id', $product_id)
                ->addFieldToFilter('pps_supplier_num', $supplier_id)
        ;
        $t_ids = array();
        ...
        //Recupere la liste
        $warehouse = Mage::getModel('AdvancedStock/Warehouse')
                ->getCollection()
    ...
    public function SaveAssociatedSupplierAction() {
        ...
        $object->setpps_wh_num($this->getRequest()->getParam('pps_wh_num'));
