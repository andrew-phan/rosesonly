...
class MDN_Purchase_OrdersController extends Mage_Adminhtml_Controller_Action {
    ...
    public function saveAction() {
        ...
        try {
            ...
            //START COSTOMIZE
            //Checking whether there is at least a product
            $currentOrderProducts = mage::getModel('Purchase/OrderProduct')
                        ->getCollection()
                        ->addFieldToFilter('pop_order_num', $this->getRequest()->getPost('po_num'));
            $currentOrderProductCount = sizeof($currentOrderProducts); //number of products orderred
            
            //get number of products will be deleted
            $productToDelete = $this->productsDataToArray($data['order_product_log']);
            $deleteProductCount = 0;
            $allProducts = $order->getProducts();
            foreach ($allProducts as $product) {
                //if we have to update datas
                if (isset($productToDelete[$product->getId()])) {
                    $currentProductData = $productToDelete[$product->getId()];
            ...
            //get number of add products
            $productsToAdd = $this->_decodeInput($this->getRequest()->getPost('add_product'));
            $productToAddCount = sizeof($productsToAdd);
            
            if (($productToAddCount + $currentOrderProductCount - $deleteProductCount) == 0) {
                Mage::getSingleton('adminhtml/session')->addError($this->__('There must be at least one product.'));
