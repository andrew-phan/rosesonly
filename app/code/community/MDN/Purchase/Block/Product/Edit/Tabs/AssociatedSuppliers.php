	...
	public function getNonLinkedSuppliersAsCombo($name='supplier')
	{
		...
		foreach ($collection as $item)
		{
			$t_ids[$item->getpps_supplier_num()]++ ;
                ...
                //Count warehouse
                $warehouses = count(Mage::getModel('AdvancedStock/Warehouse')->getCollection());
		...
		{
			//$retour .= '<option value="'.$item->getId().'">'.$item->getsup_name().'</option>';
                        if($t_ids[$item->getId()] < $warehouses)
                            $retour .= '<option value="'.$item->getId().'">'.$item->getsup_name().'</option>';
		}
		$retour .= '</select>';
        ...
        /// Phuoc's code
        public function getNonLinkedWarehouseAsCombo($supplierId)
	{
		$collection = mage::GetModel('Purchase/ProductSupplier')
				->getCollection()
				->addFieldToFilter('pps_product_id', $this->getProduct()->getId())
		...
		foreach ($collection as $item)
		{
			$t_ids[] = $item->getpps_wh_num();
		}
		
		//Recupere la liste
		$collection = mage::GetModel('AdvancedStock/Warehouse')
						->getCollection()
						->addFieldToFilter('stock_id', array('nin' => $t_ids))
						->setOrder('stock_name', 'asc');
		
		//transforme en combo
		$retour = '<select id="pps_wh_num" name="pps_wh_num">';
		foreach($collection as $item)
		{
			$retour .= '<option value="'.$item->getId().'">'.$item->getStock_name().'</option>';
		}
		$retour .= '</select>';
