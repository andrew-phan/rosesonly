...
class MDN_Purchase_Helper_Product_Cost extends MDN_AdvancedStock_Helper_Product_Cost
{
	...
	public function getProductCostAtDate($product, $date, $qty, $warehouse)
	{
		...
		if ($priceCount > 0)
			$retour = number_format($priceSum / $priceCount, 2);		
		if ($retour == 0)
			$retour = parent::getProductCostAtDate($product, $date, $qty, $warehouse);
		if ($sourceCount > 1)
			$this->_comments .= 'Product '.$productName.' stock comes from '.$sourceCount.' sources (price = '.$retour.')'."\n";
		if ($sourceCount == 0)
			$this->_comments .= 'Product '.$productName.' stock comes from 0 sources (price = '.$retour.')'."\n";
