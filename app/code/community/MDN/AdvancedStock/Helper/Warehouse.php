...
class MDN_AdvancedStock_Helper_Warehouse extends Mage_Core_Helper_Abstract
{
	...
	public function getStockAtDateContent($warehouseId, $date)
	{
		...
		foreach ($collection as $product)
		{
			...
			//append line
			$content .= $product->getAttributeText('manufacturer').';'.$product->getId().';'.$product->getSku().';'.$product->getName().';'.$cost.';'.$stockLevel.';'.$status.self::_lineReturn;
		}
