...
class MDN_AdvancedStock_Model_StockMovement extends Mage_Core_Model_Abstract {
    ...
    protected function _afterDelete() {
        ...
        $productId = $this->getsm_product_id();
        mage::helper('BackgroundTask')->AddTask('Update stocks for product #' . $productId . ' (stock mvt deleted)', 'AdvancedStock/Product_Base', 'updateStocksFromProductId', $productId
        );
    }
     ...
     * @param unknown_type $additionalDatas
     */
        ...
        try {
            ...
            if ($sourceWarehouseId != null) {
                Mage::dispatchEvent('product_supplier_sub', array('productid' => $productId,
                    'qty' => $qty,
                    'whid' => $sourceWarehouseId));
            }
            if ($targetWarehouseId != null) {
                Mage::dispatchEvent('product_supplier_add', array('productid' => $productId,
                    'qty' => $qty,
                    'supplierid' => $supplierId,
                    'whid' => $targetWarehouseId));
            }
            ///////
