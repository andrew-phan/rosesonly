        ...
        foreach ($collection as $item) {
            if (($item->getexclude() == 1) || ($quote->getshow_detail_price() == 1)) {
                ...
                if ($quote->getshow_detail_price() || ($item->getexclude() == 1)) {
                    ...
                    $this->drawTextInBlock($page, $quote->FormatPrice($item->GetUnitPriceWithoutTaxes($quote)), 490, $this->y, 60, 20, 'r');
                    $this->drawTextInBlock($page, $quote->FormatPrice($item->GetTotalPriceWithoutTaxes($quote)), 560, $this->y, 60, 20, 'r');
