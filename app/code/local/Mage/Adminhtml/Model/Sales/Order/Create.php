...
class Mage_Adminhtml_Model_Sales_Order_Create extends Varien_Object implements Mage_Checkout_Model_Cart_Interface
{
    ...
    public function updateOrder($orderId)
    {
        ...
        $service = Mage::getModel('sales/service_quote', $quote);
        $c_order = Mage::getModel('sales/order')->load($orderId);
        if ($this->getSession()->getOrder()->getId()) {
            $orderData = array(
//                'entity_id' => $c_order->getData("entity_id"),
                'original_increment_id'     => $c_order->getData("original_increment_id"),
                'relation_parent_id'        => $c_order->getData("relation_parent_id"),
                'relation_parent_real_id'   => $c_order->getData("relation_parent_real_id"),
                'edit_increment'            => $c_order->getData("edit_increment"),
                'increment_id'              => $c_order->getData("increment_id")
            );
            $quote->setReservedOrderId($orderData['increment_id']);
            $service->setOrderData($orderData);
        }
//        $c_order->delete();
        $this->deleteOrder($c_order);
        $order = $service->submit();
        if ((!$quote->getCustomer()->getId() || !$quote->getCustomer()->isInStore($this->getSession()->getStore()))
            && !$quote->getCustomerIsGuest()
        ) {
