...
class Mage_Adminhtml_Block_Sales_Order_View extends Mage_Adminhtml_Block_Widget_Form_Container
{
    public function __construct()
    {
        ...
        // Hau Vo: add button to change state from draft to normal order
        if($order->getData('is_draft') === '1')
        {
            $this->_addButton('submit', array(
                    'label'     => Mage::helper('sales')->__('Submit'),
                    'onclick'   => 'setLocation(\'' . $this->getSubmitUrl() . '\')',
                    'class'     => 'save',
                ));
        }
        
        $this->_addButton('redelivery', array(
                    'label'     => Mage::helper('sales')->__('Redelivery'),
                    'onclick'   => 'setLocation(\'' . $this->getRedeliveryUrl() . '\')',
                    'class'     => 'go',
                ));
        ...
        $this->_addButton('so_print', array(
                'label'     => Mage::helper('sales')->__('SO'),
                'onclick'   => 'window.open(\'' .$this->getSOPrintUrl(). '\')',
                'target'    => '_blank',
                'class'     => 'save'
        ));
    ...
    // change state of order from draft to normal
    public function getSubmitUrl() 
    {
        return $this->getUrl('*/sales_order_edit/changeToDraft');
    }
    ...
    {
        return  Mage::helper("adminhtml")->getUrl('deliverymanagement/adminhtml_deliverymanagement/index');
    }
    ...
    public function getSOPrintUrl()
    {
        return $this->getUrl('*/*/soprint', array(
            'order_id' => $this->getOrderId()            
        ));
    }
    ...
    public function getRedeliveryUrl()
    {
        // Hau Vo: link redelivery
        return $this->getUrl('*/sales_order_redelivery/start');
//        , array(
//            'order_id' => $this->getOrderId()            
//        ));
    }
