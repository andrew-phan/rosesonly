    ...
    implements Mage_Catalog_Model_Resource_Product_Indexer_Price_Interface
{
    ...
    protected function _applyCustomOption()
    {
        ...
        $optPriceValue  = $write
                //->getCheckSql('otps.option_type_price_id > 0', 'otps.price', 'otpd.price')
                ->getCheckSql('(ot.special_from IS NULL OR ot.special_from <= CURDATE())
                    AND (ot.special_to IS NULL OR ot.special_to >= CURDATE()) 
                    AND (ot.special_from IS NOT NULL OR ot.special_to IS NOT NULL)',
                    'ot.special_price', 'otpd.price')
                ;
