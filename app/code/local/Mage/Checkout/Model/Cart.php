    ...
    public function updateItems($data)
    {
        ...
        foreach ($data as $itemId => $itemInfo) {
            ...
            if(!empty($itemInfo['comments'])) {
                $write = Mage::getSingleton('core/resource')->getConnection('core_write'); //die;
                # make the frame_queue active
                $query = "UPDATE sales_flat_quote_item SET custom_attribute = '".$itemInfo['comments']."' where item_id = $itemId";
                $write->query($query);
                $item->setCustom_attribute($itemInfo['comments']);
           }
