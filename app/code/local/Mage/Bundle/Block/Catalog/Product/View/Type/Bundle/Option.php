...
class Mage_Bundle_Block_Catalog_Product_View_Type_Bundle_Option extends Mage_Bundle_Block_Catalog_Product_Price
{
    ...
    public function getSelectionQtyTitlePrice($_selection, $includeContainer = true)
    {
        ...
        $priceTitle = '<div style="margin-top: 3px; margin-left: 10px">';
        $priceTitle .= '&nbsp;'.($includeContainer ? '<span class="price-notice-custom" >' : '')
            . $this->formatPriceString($price, $includeContainer)
            . ($includeContainer ? '</span><br/>' : '');

        $priceTitle .= '<div class="add-on-title">'.$this->escapeHtml($_selection->getName()).'</div>';
