...
class MDN_Purchase_Model_Pdf_Order extends MDN_Purchase_Model_Pdf_Pdfhelper {
    ...
    public function getPdf($orders = array()) {
        ...
        foreach ($orders as $order) {
            ...
            foreach ($order->getProducts() as $item) {
                ...
                $caption = $this->WrapTextToWidth($page, $item->getpop_product_name(), 170);
                ...
                if ($order->getpo_status() != MDN_Purchase_Model_Order::STATUS_INQUIRY) {
                    ...
                    $this->drawTextInBlock($page, number_format($item->getpop_tax_rate(), 2) . '%', 400, $this->y, 40, 20, 'c');
            ...
            */
            ...
            //Rajoute le r�glement et le transporteur
            $page->setFont(Zend_Pdf_Font::fontWithName(Zend_Pdf_Font::FONT_HELVETICA), 10);
            $this->y -= 20;
    ...
    public function drawTableHeader(&$page) {
        ...
        $page->drawText(mage::helper('purchase')->__('Unit Price'), 283, $this->y, 'UTF-8');
        $page->drawText(mage::helper('purchase')->__('Qty'), 345, $this->y, 'UTF-8');
        //$page->drawText(mage::helper('purchase')->__('WEEE'), 365, $this->y, 'UTF-8');
        $page->drawText(mage::helper('purchase')->__('Tax'), 400, $this->y, 'UTF-8');
        $page->drawText(mage::helper('purchase')->__('Subtotal'), 453, $this->y, 'UTF-8');
        $page->drawText(mage::helper('purchase')->__('Total'), 523, $this->y, 'UTF-8');
