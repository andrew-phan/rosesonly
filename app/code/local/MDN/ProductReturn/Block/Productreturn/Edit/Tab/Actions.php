 ...
 */
class MDN_ProductReturn_Block_Productreturn_Edit_Tab_Actions
    extends Mage_Adminhtml_Block_Widget_Form
{
	...
	public function getPaymentMethodAsCombo($name, $value='')
	{
		...
		foreach($paymentmethods as $method)
		{
			...
			$retour .= '<option value="'.$method->getId().'" '.$selected.'>'.$label.'</option>';
		}
		$retour .= '</select>';
		return $retour;	
				...
				if ($item['model'])
				{
					...
					if ($allowedMethods)
					{
						foreach ($allowedMethods as $methodKey => $method)
						{
							...
							if (strlen($label) > 50)
								$label = substr($label, 0, 50).'...';
							$retour .= '<option value="'.$finalKey.'" '.$selected.'>'.$label.'</option>';
	...
	{
		return $this->getUrl('ProductReturn/Admin/ProductExchangeSelectionPopup', array('rp_id' => 'XXX'));
	}
	...
	/**
	 * Return tax mode for shipping & technical fees
	 *
	 */
