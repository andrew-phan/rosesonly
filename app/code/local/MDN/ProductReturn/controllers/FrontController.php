...
class MDN_ProductReturn_FrontController extends Mage_Core_Controller_Front_Action
{
	...
	public function SubmitRequestAction()
	{
	    ...
	    //on va verifier si il ya des quantités pour au moin 1 produit.
	...
	public function ViewAction()
	{
		...
		//on verifie si le rma correspond a customer connecté
	...
	public function PrintPdfAction()
	{
		...
		//on verifie si le rma correspond au customer connecté
		$RmaId = $this->getRequest()->getParam('rma_id');
		$rma = mage::getModel('ProductReturn/Rma')->load($RmaId);
		if ($this->RmaBelongToCustomer($rma->getrma_customer_id()) === true)
    	{
			try
		    	...
		    	{
		    		die("Erreur lors de la génération du PDF: ".$ex->getMessage()."<br>".$ex->getTraceAsString());
		    	}
    	}
    	else {
    		$this->_redirect('');		
