...
class MDN_ProductReturn_AdminController extends Mage_Adminhtml_Controller_Action
{
    ...
    public function SaveAction()
    {
    	...
    	try
    	{
	    	...
	    	foreach ($rmaProducts as $rmaProduct)
	    	{
	    		if (mage::getModel('ProductReturn/RmaProducts')->productIsDisplayed($rmaProduct))
	    		{
			    	...
			    	else 
			    	{
		   			...
		   			$rmaProductItem = $rma->updateSubProductInformation($rmaProduct, $qty, $description, $reason, $serials);  				
	    		}
	    	}
	    	...
	    	if (!$isCreation)
	    	{
		    	...
		    	/*
		    	echo '<pre>';
		    	var_dump($this->_OrderCreationData);
		    	echo '</pre>';
		    	die('');
		    	*/
	...
	private function initDataOrder($rma){
		...
		if ($data['rma_technical_cost'] > 0)
		{
			...
			if (mage::getStoreConfig('tax/calculation/price_includes_tax'))
			{
				...
				$priceHt = $helper->getPrice($fakeProduct, $price, false, $ShippingAddress, $BillingAddress, $CustomerTaxClass, $storeId);				
			}
			else 
			{
				$priceHt = $data['rma_technical_cost'];	
				$price = $helper->getPrice($fakeProduct, $priceHt, true, $ShippingAddress, $BillingAddress, $CustomerTaxClass, $storeId);				
			}
		...
		$this->_OrderCreationData['store_id'] = $rma->getCustomer()->getstore_id();
		$this->_OrderCreationData['website_id'] = $rma->getCustomer()->getwebsite_id();
		...
		if (mage::getStoreConfig('tax/calculation/price_includes_tax'))
		{
			$this->_OrderCreationData['shipping_cost'] = $helper->getShippingPrice($data['rma_shipping_cost'], false, $rma->getCustomerAddress());		
			$this->_OrderCreationData['shipping_taxamount']	= $data['rma_shipping_cost'] - $this->_OrderCreationData['shipping_cost'];
			
		}
		else 
		{
			$this->_OrderCreationData['shipping_cost'] = $data['rma_shipping_cost'];
			$this->_OrderCreationData['shipping_taxamount'] = ($helper->getShippingPrice($data['rma_shipping_cost'], true, $rma->getCustomerAddress()) - $data['rma_shipping_cost']);			
		}
	...
	private function initDataCreditMemo($rma)
	{
		...
		foreach ($rma->getProducts() as $item)
		{
			...
			//check if product action is refund
			if ($this->getActionForProduct($rpId) != 'refund')
				continue;
