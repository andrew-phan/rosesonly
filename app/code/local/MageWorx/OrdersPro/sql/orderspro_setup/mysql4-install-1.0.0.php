...
-- DROP TABLE IF EXISTS {$this->getTable('orderspro_order_group')};
CREATE TABLE IF NOT EXISTS {$this->getTable('orderspro_order_group')} ( 
...
INSERT IGNORE INTO {$this->getTable('orderspro_order_group')} (`order_group_id`, `title`) VALUES
(1, 'Archived'),
(2, 'Deleted');


-- DROP TABLE IF EXISTS {$this->getTable('orderspro_order_item_group')};
CREATE TABLE IF NOT EXISTS {$this->getTable('orderspro_order_item_group')} (
  ...
  CONSTRAINT `FK_ORDERSPRO_ORDER_GROUP_ID` FOREIGN KEY (`order_group_id`) REFERENCES `{$this->getTable('orderspro_order_group')}` (`order_group_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB DEFAULT CHARSET=utf8 COMMENT='OrdersPro Order Item Group';

  
-- DROP TABLE IF EXISTS {$this->getTable('orderspro_upload_files')};
CREATE TABLE IF NOT EXISTS {$this->getTable('orderspro_upload_files')} (
