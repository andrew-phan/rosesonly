...
class MW_FreeGift_Adminhtml_Freegift_CatalogController extends Mage_Adminhtml_Controller_Action
{
	...
	protected function _initAction() {
		$this->loadLayout()
			 ->_setActiveMenu('mageworld/freegift/catalog')
			->_addBreadcrumb(Mage::helper('adminhtml')->__('Items Manager'), Mage::helper('adminhtml')->__('Catalog Rules Manager'));
	...
	public function saveAction() {
		if ($this->getRequest()->getPost()) {
            try {
				...
				$customer_group_ids = "";
				$website_ids = "";
				if(isset($data["customer_group_ids"])) $customer_group_ids = implode(",",$data["customer_group_ids"]);
				if(isset($data["website_ids"])) $website_ids = implode(",",$data["website_ids"]);
    ...
    protected function _removeOlderData($rule_id = null)
    {
    	...
    	if($rule_id) $collection->addFieldToFilter('rule_id',$rule_id);
