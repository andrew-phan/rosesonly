    ...
    protected function _prepareCollection() {
        ...
        if (version_compare(Mage::getVersion(), '1.4.1.0', '>=')) {
	        ...
	        $collection = Mage::getResourceModel('sales/order_collection')
                    ...
                    ->addExpressionAttributeToSelect('shipping_name', 'CONCAT({{shipping_firstname}}, " ", {{shipping_lastname}})', array('shipping_firstname', 'shipping_lastname'));
            $collection->getSelect()->joinleft(//array(	'one_step'=>'mw_onestepcheckout'),
                    array('one_step' => $collect->getTable('mw_onestepcheckout')), 'one_step.sales_order_id=e.entity_id', array('mw_deliverydate_date', 'mw_customercomment_info', 'mw_deliverydate_time')
            );
            $this->setCollection($collection);
        }
...
//                    if ($field == 'mw_customercomment_info' OR $field == 'mw_deliverydate_date' OR $field == 'mw_deliverydate_time') {
                    if ($field == 'mw_customercomment_info' OR $field == 'mw_deliverydate_time') {
                        ...
                        if (version_compare(Mage::getVersion(), '1.4.1.0', '>=')) {
                            if($field == 'mw_deliverydate_date')
                                $this->getCollection()->addFieldToFilter("one_step." . $field, $cond);
                            else $this->getCollection()->addFieldToFilter("main_table." . $field, $cond);
                        } else {
                            $this->getCollection()->addFieldToFilter($field, $cond);
                        }
                    }
