...
$countrygeo=Mage::registry('Countrycode');
  ...
  //  $this->getQuote()->save();
    ...
    public function updateemailmsgAction(){
            ...
            // Hau comment this code, dont check email at here, always return true
//		if($customer->getId()){
...
//		}
    ...
    public function updateshippingtypeAction(){	$this->notshiptype=1;
/*$data=$this->getRequest()->getPost($isbilling,array());*/
   ...
   // $this->getResponse()->setRedirect(Mage::getUrl('*/*/forgotpassword'));
    ...
    // Hau Vo: this function for process new order from front end
    public function updateordermethodAction(){
        if(!$this->isCustomerLoggedIn()){
//            $log_msg .="\n------: User check out as a guest";
            if($isguest=='1' or Mage::helper('onestepcheckout')->haveProductDownloadable()){
                /*neu checkbox regiter_new_accoutn dc check or co san pham downloadable thi tao new acc*/
...
//        $log_msg .="\nCheck request is POST?";
        ...
        if ($data_order = $this->getRequest()->getPost('payment', false)) {
            ...
            /* send mail after save delivery time*/
            try{	//kiem tra xem cac info nhu info payment co' phai? hop le hay la` gian lan, neu ko thi se bat loi~ =catch va` redirect

                $onepage_order = $this->getOnepage()->saveOrder();
                /* Hoang check order delivery*/

                $current_order = Mage::getModel('sales/order')->loadByIncrementId($onepage_order->getLastOrderId());

                /* Add Track To Purchase */

                
                /*$order_id = $current_order->getRealOrderId();
                $total = number_format($current_order->getGrandTotal(), 2);
                ...
                Mage::getModel('baobaz_ems/webservice_subscribers')->Track_To_Purchase($order_id, $total, $items, $currency, $country);*/

                

                /* End add Track To Purchase */


                $data_customercomment = $this->getrequest()->getpost('onestepcheckout_comments');
                //$date = new DateTime();
            //    $Deliverydate = $date->createFromFormat('d/m/Y', $this->getrequest()->getpost('_deliverydate'));
            //    $Deliverytime= $this->getrequest()->getpost('onestepcheckout_time');
                $order=Mage::getModel('onestepcheckout/onestepcheckout');

                $order->setSalesOrderId($current_order->getEntityId());
         ...
         //       $order->setMwDeliverydateTime($Deliverytime);
                ...
                // Hau added: need uncomment when deploy
                //fix email order
                $current_order->setData('billing_email_guest',$data_save_billing['email']);

                if($this->getRequest()->getPost("is_add_to_mailing_list") === 'on'){
                    $this->addToNewsletter($data_save_billing['email']);
                }
                
                $session_ = Mage::getSingleton('core/session');
                $hhonors_number = $session_->getData('hhonors_number');
                if($hhonors_number)
                {
                    $hhonors_bonus_code = $session_->getData('hhonors_bonus_code');
                    $current_order->setData('order_hhonors_number', $hhonors_number);
                    $current_order->setData('order_hhonors_bonus_code', $hhonors_bonus_code);
                    $customer_id = $session_->getData('hhonors_customer_id');
                    if($customer_id)
                    {
                        $customer = Mage::getModel('customer/customer')->load($customer_id);
                        $hhonor_point = $customer->getData('hhonors_point');
                        $new_hhonor_point = $hhonor_point;
                        $new_hhonor_point+=1000;
                        $customer->setData('hhonors_point',$new_hhonor_point);
                        $customer->save();
                        $session_->unsetData('hhonors_customer_id');
                    }
                    
                    $session_->unsetData('hhonors_number');
                    $session_->unsetData('hhonors_bonus_code');
                }
                $customer_email = $current_order->getCustomerEmail();
                if(strlen(trim($customer_email)) == 0 ){
                    $current_order->setCustomerEmail($data_save_billing['email']);
                    //$current_order->save();
    ...
    private function logMsg($msg){
        Mage::log($msg, null, 'new_order.log');
    }


    //add to newsletter function
    private function addToNewsletter($email){
...
//            }
            ...
            $status = Mage::getModel('newsletter/subscriber')->subscribe($email, false);
            if ($status == Mage_Newsletter_Model_Subscriber::STATUS_NOT_ACTIVE) {
                $session->addSuccess($this->__('Confirmation request has been sent.'));
            }
            else {
                //$session->addSuccess($this->__('Thank you for your subscription.'));
                //return $this->_redirectURL('/thankyou');
            }
...
//		die();
    ...
    public function giftmessageAction(){
        ...
        $block = $this->getLayout()->createBlock(
            ...
            'MW_Onestepcheckout_Block_Checkout_onepage_Giftmessage',
            array('template' => 'mw_onestepcheckout/daskboard/onepage/giftmessage.phtml')
        );
