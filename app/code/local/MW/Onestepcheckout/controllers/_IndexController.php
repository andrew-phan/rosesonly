   ...
   // $this->getResponse()->setRedirect(Mage::getUrl('*/*/forgotpassword'));
    ...
    public function updateordermethodAction(){
        ...
        if ($this->getrequest()->ispost()) {
                ...
                $__orders = Mage::getModel('onestepcheckout/onestepcheckout')->getCollection()
                            ->setOrder('sales_order_id','DESC')
                            ->setPageSize(1)
                            ->setCurPage(1);

                $_lastOrderId2 = $__orders->getFirstItem()->getSalesOrderId();

                /*
                if ($lastOrderId > ($_lastOrderId2 + 1))
                    $_lastOrderId = $lastOrderId;
                else {
                    $_lastOrderId = $_lastOrderId2 + 1;
                }
                */

                $_lastOrderId = $_lastOrderId2+1;

                $order->setSalesOrderId($_lastOrderId);
            ...
            try{	//kiem tra xem cac info nhu info payment co' phai? hop le hay la` gian lan, neu ko thi se bat loi~ =catch va` redirect
                ...
                $c_orders = Mage::getModel('sales/order')->getCollection()
                                ->setOrder('created_at','DESC')
                                ->setPageSize(1)
                                ->setCurPage(1);
                
                
                $current_order = $c_orders->getFirstItem();
                ...
                if($hhonors_number)
                {
                    ...
                    $date = (int)(date('Y')/100).strftime('%y%j');
                    $time= date("His");
                    $file_name = "RSES".$date."_".$time.".dat";
                    $content  = '0'.'RSES'.'REG'.'HH'.$date.$time.PHP_EOL
                                .'1'.'T'.'RSES'.'TOOH'.$date.PHP_EOL;
                    ...
                    foreach($items as $item) {
                        ...
                        if($_mproduct->getTypeId() == "bundle"){
                            $count ++;
                            $content .= '2'.str_pad($current_order->getIncrementId(),16,"0",STR_PAD_LEFT).'   '.$hhonors_number.$data_save_billing['lastname'].str_repeat(' ', 19)
                                            .str_pad((int) 1000,7,"0",STR_PAD_LEFT).'99'.$_mproduct->getSku().'    '.$hhonors_bonus_code.PHP_EOL;
                        }
                    }   
                ...
                // echo 's';
                $c_lastOrderId = $c_orders->getFirstItem()->getEntityId();
                // echo '3';   
                $m_order=Mage::getModel('onestepcheckout/onestepcheckout')->getCollection()
                                ->setOrder('sales_order_id','DESC')
                                ->setPageSize(1)
                                ->setCurPage(1);
                //echo '3';
                $m_checkout= $m_order->getFirstItem();
                
                
                
                //echo 'e';
                if($m_checkout->getSalesOrderId() != $c_lastOrderId){
                    $m_checkout->setSalesOrderId($c_lastOrderId);
